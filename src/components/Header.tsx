...
const Header: React.FC<MenuProps> = ({items}) => {
    ...
    return (
        <header className={`fixed top-0 w-full h-16 flex z-50 px-10 justify-center items-center ${isScrolled ? 'bg-gray-900 shadow-md' : 'bg-transparent'}`} aria-label="Main Navigation">
            <nav className="flex justify-end items-center transition-colors duration-300 text-white w-full" role="navigation">
                ...
                <ul className={`py-2 md:flex flex-col md:flex-row justify-center w-full ${isMenuOpen ? 'flex absolute top-16 left-0 right-0 bg-gray-900' : 'hidden md:flex'}`} role="menubar">
